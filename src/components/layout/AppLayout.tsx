...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const showBanner =
    !!activeTraining && location.pathname !== `/trainings/${activeTraining.id}`;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Main Content */}
      <main className={cn('flex-1 w-full max-w-md mx-auto p-4', showBanner ? 'pb-44' : 'pb-28')}>
        {children}
      </main>

      {/* Bottom Navigation (+ Banner zum aktiven Training) */}
      <div className="fixed bottom-0 left-0 right-0">
        {showBanner && activeTraining && <ActiveTrainingBanner training={activeTraining} />}
        <nav className="rounded-t-[32px] bg-white/80 backdrop-blur-xl shadow-[0_-4px_24px_rgba(0,0,0,0.04)]">
