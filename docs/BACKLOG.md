...
- **Minimales Rollenkonzept (Admin/User) — Katalog schützen:** Heute darf **jeder** freigeschaltete User den globalen Katalog anlegen UND bearbeiten (keine Admin-Rolle in den Rules). Da der Katalog global ist, wirken Änderungen eines Testers für alle. Soll geändert werden:
  - **Erster Schritt (bevorzugt, minimal):** User dürfen den Katalog **gar nicht** schreiben — kein Bearbeiten, kein Hinzufügen. Nur der **Admin** (Josef) pflegt den Katalog. Begründung: etablierte Apps machen es genauso; der Seed (50 Übungen) ist umfangreich genug. Am 2026-06 mit Angela live getestet — alles, was sie braucht, ist bereits da. **Aufwand klein:** Rules-Change, der `create`/`update` auf `exercises` auf die Admin-Identität (UID oder E-Mail) beschränkt; das Bearbeiten-UI in `Exercises.tsx` für Nicht-Admins ausblenden. Löschen wird damit auch unkritisch (nur Admin).
